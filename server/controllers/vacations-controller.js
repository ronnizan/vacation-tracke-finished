...
router.post("/", authMiddleware, async (req, res) => {
  try {
    ...
    res.status(201).json({ msg: "successfully added vacation!" });
  } catch (error) {
    res.status(500).send("server error");
  }
});
...
router.patch("/update", authMiddleware, async (req, res) => {
  try {
    ...
    const allVacationUpdated = await vacationLogic.getAllVacations();
    const msg =
      "Admin has updated " + receivedVacation.destination + " vacation deatils";
    ...
    res.status(201).json({ msg: "successfully edited vacation!" });
  } catch (error) {
    res.status(500).send("server error");
  }
});
    ...
    fs.unlink(
      ...
      async (err) => {
        if (err) {
          ...
          const allVacationUpdated = await vacationLogic.getAllVacations();
          const msg = "Admin has removed the vacation to " + vacationName;
          global.socketServer.sockets.emit(
            "admin-change",
            allVacationUpdated,
            req.user.isAdmin,
